...
/*
 *	Function that print data via UART
 */
int _write(int fd, unsigned char *buf, int len) {
  if (fd == 1 || fd == 2) {
    HAL_UART_Transmit(&huart4, buf, len, 999);  // Print to the UART
  }
  return len;
}
...
int main(void)
{
  ...
  /* !ONLY test speed for read / write. for some project use sd_function */
  sd_benchmark();
